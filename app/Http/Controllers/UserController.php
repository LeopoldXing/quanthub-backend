...
class UserController extends Controller
{
    public function createMyUser(Request $request) {
        ...
        $validated = $request->validate([
            'auth0Id' => 'required|string|unique:quanthub_users,auth0Id',
            'email' => 'required|email|unique:quanthub_users,email',
        ]);

        // 检查用户是否存在
        $user = QuanthubUser::where('auth0Id', $validated['auth0Id'])
    ...
    public function updateProfile(Request $request) {
        ...
        return response()->json($user, 200);  // 将用户对象转换为JSON并返回
    }
    ...
    public function getUserProfile(Request $request) {
        ...
        if (!empty($validated['auth0Id'])) {
            $user = QuanthubUser::where('auth0Id', $validated['auth0Id'])->first();
